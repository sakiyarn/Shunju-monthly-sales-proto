...
class StaffMonthlyResultsController < ApplicationController
  def bulk_upsert
    perform_bulk_upsert(editable_user_ids: editable_member_user_ids, notice_message: "S7を保存しました")
  end

  def bulk_upsert_officers
    perform_bulk_upsert(editable_user_ids: editable_officer_user_ids, notice_message: "S8を保存しました")
  end
  ...
  def perform_bulk_upsert(editable_user_ids:, notice_message:)
    ...
    redirect_to master_path, notice: notice_message
  rescue ArgumentError => e
    redirect_with_errors(entries: e.message)
  end

  def editable_member_user_ids
    User.where(is_active: true, system_role: "member").pluck(:id).to_set
  end
