<template>
  <main class="mx-auto max-w-[1400px] space-y-6 px-6 py-8">
    <section class="card">
      ...
      <button class="btn-accent mb-4" @click="openUserModal()">＋ メンバーを追加</button>
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="text-left text-slate-600">
              <th class="p-2">名前</th><th class="p-2">メール</th><th class="p-2">権限</th><th class="p-2">操作</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(u, i) in users" :key="u.id" class="border-t border-slate-200">
              <td class="p-2">{{ u.name }}</td>
              <td class="p-2">{{ u.email }}</td>
              <td class="p-2">{{ u.role }}</td>
              <td class="p-2"><button class="btn-sub" @click="openUserModal(i)">編集</button></td>
            </tr>
          </tbody>
        </table>
      </div>
      <p v-if="errors.s1" class="error-msg">{{ errors.s1 }}</p>
      <button class="btn-save" @click="saveSection('s1')">保存</button>
    </section>
    ...
    <section class="card">
      ...
      <div class="ag-theme-quartz h-[320px] w-full">
        <AgGridVue class="h-full w-full" :rowData="workRowsByProject[activeWorkProjectId]" :columnDefs="workColDefs" :defaultColDef="defaultColDef" :frameworkComponents="frameworkComponents" @cell-value-changed="onWorkCellChanged" />
      </div>
    ...
    <div v-if="userModal.open" class="fixed inset-0 z-50 grid place-items-center bg-black/60">
      <div class="w-full max-w-md rounded-lg border border-slate-200 bg-card p-4">
        <h3 class="mb-3 text-lg font-semibold">{{ userModal.editingIndex === null ? 'メンバー追加' : 'メンバー編集' }}</h3>
        <div class="space-y-2">
          ...
          <select v-model="userModal.form.role" class="input w-full">
...
const frameworkComponents = { pairCellEditor: PairCellEditor }
...
function openUserModal(index?: number) {
  if (typeof index === 'number') {
    ...
    userModal.editingIndex = index
  } else {
    userModal.form = { name: '', email: '', role: 'member' }
    userModal.editingIndex = null
  }
  userModal.open = true
}

function closeUserModal() { userModal.open = false }

function saveUserModal() {
  if (!userModal.form.name || !userModal.form.email) return
  if (userModal.editingIndex === null) {
    users.push({ id: `u${String(users.length + 1).padStart(2, '0')}`, ...userModal.form })
  } else {
    users[userModal.editingIndex] = { ...users[userModal.editingIndex], ...userModal.form }
  }
